...
public class UserStatistics_dailyupdate {
	...
	* @throws SQLException
	**/
	public static final String EVENT_TABLE = "github_events_hive_daily";// //github_events_hive_daily
	...
	public static void updateToMongo(DBCollection coll, String user,String field, String fieldValue, int value){
		...
		if(user!=null && field!=null && fieldValue!=null){	
			// find the user
			...
			if (cur.count()==0){
				...
				//System.out.println(user+"--"+field+"--"+fieldValue+"--"+value);
				
				coll.insert(newDoc);
			}else{
				// update
				BasicDBObject newDoc  = new BasicDBObject();
				newDoc.append("$inc", new BasicDBObject().append((field+"."+fieldValue), value));
	...
	public static void main(String[] args) throws SQLException {
		...
		// mongodb db
		DB db = mongo.getDB("github_users");//github_users
